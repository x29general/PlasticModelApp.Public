...
3. ユーザーが、色の類似度の閾値（任意、未設定時はデフォルト値5）を設定する。
4. ユーザーが「検索」ボタンをクリックする。
5. システムが選択された色に近い塗料を表示する。
6. ユーザーは任意で、「次へ」「前へ」ボタンでページ移動ができる。
...
1. ユーザーが塗料検索ページにアクセスする。
2. ユーザーが色を選択しない場合、システムが、検索ボタンを非活性化する。
3. ユーザーが「検索」ボタンをクリックできない。
